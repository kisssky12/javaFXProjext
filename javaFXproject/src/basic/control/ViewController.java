...
public class ViewController implements Initializable {
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		tcSmartPhone.setCellValueFactory(new PropertyValueFactory<>("smartPhone")); // smartPhone을 (set) CellValue값에
																					// 담겟다.
		...
		tableView.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<Phone>() {
			...
			public void changed(ObservableValue<? extends Phone> obs, Phone oldV, Phone newV) {
				imageView.setImage(new Image("basic/image/" + newV.getImage()));
			}
		});
	}
}
